...
def calculate_with_dead_models(curr_file_loader,dead_outlier_model,curr_set,curr_obs_params,LABEL):
    ...
    for i, files in enumerate(curr_file_loader.filelists):
        ...
        if get_file not in curr_obs_params:
            ...
            #print(f"current bayesian model stats: {dead_outlier_model.n}\n {dead_outlier_model.mu}\n {dead_outlier_model.cov_matrix}")
...
def run_outlier_model():
    ...
    file_loader.load_files_from_folder(ALIVE,9)
    ...
    for files in file_loader.filelists:
        ...
        #print(f"alive model params: {dead_grid_displacement_model.n}")
    ...
    #plot_histogram(dead_train_probability_set, alive_train_probability_set)
...
def run_bayesian_model():
    ...
    first_key, first_value = next(iter(train_with_dead_params_probability_set.items()))
    #print(f" probability set: {first_key}, {first_value}")
    '''
    obs_key, obs_value = next(iter(train_set.items()))
    print(f" obs set: {obs_key}, {obs_value}")
    '''
    ...
    prob_key, prob_value = next(iter(train_with_alive_params_probability_set.items()))
    #print(f" probability set: {prob_key}, {prob_value}")
    
    bayesian_evaluate_model = BayesianModel()   
    
    bayesian_evaluate_model.calculate_prior(dead_with_alive_train_probability_set, alive_with_alive_train_probability_set)   
    print(f"{bayesian_evaluate_model.prior_dead}, {bayesian_evaluate_model.prior_alive}")
    
    train_bayesian_probability_set=bayesian_evaluate_model.sum_log_probabilities(train_with_dead_params_probability_set, train_with_alive_params_probability_set)
    obs_key, obs_value = next(iter(train_bayesian_probability_set.items()))
    
    bayesian_evaluate_model.plot_confusion_matrix(train_bayesian_probability_set, "Train")
    '''
    model_evaluator_visualization= OutlierModelEvaluation()
    alive_train_ids=model_evaluator_visualization.find_the_needed_obj_id(train_bayesian_probability_set,DEAD,DEAD)
    model_evaluator_visualization.plot_extracted_obj(alive_train_ids,train_set,DEAD,DEAD)
    '''
    #testing begins:
    ...
    test_bayesian_probability_set=bayesian_evaluate_model.sum_log_probabilities(test_with_dead_params_probability_set, test_with_alive_params_probability_set)
    #obs_key, obs_value = next(iter(train_bayesian_probability_set.items()))
    
    bayesian_evaluate_model.plot_confusion_matrix(test_bayesian_probability_set, "Test")
    '''
    alive_test_ids=model_evaluator_visualization.find_the_needed_obj_id(test_bayesian_probability_set,ALIVE,ALIVE)
    model_evaluator_visualization.plot_extracted_obj(alive_test_ids,test_set,ALIVE,ALIVE)
    '''
