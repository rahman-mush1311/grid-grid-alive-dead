...
class GridDisplacementModel:
    ...
    def calculate_displacements(self, observations):
        ...
        if  len(observations)>1:
            ...
            print(f"The observations comes from dead files no normalization for alive in that files")
        ...
        for row in range(len(grid_displacements)):
            for col in range(len(grid_displacements[row])):
                ...
                if grid_displacements[row][col]:  # Only normalize if the cell is not empty
                    ...
                    print(f"from apply normalization function: [{row}][{col}] doesn't contain any element {len(grid_displacements[row][col])}")
