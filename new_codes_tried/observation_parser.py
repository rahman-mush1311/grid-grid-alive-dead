...
class ParsingObservations:
    ...
    def load_observations(self,file_path_filename):
        """
        ...
        prefix, extension = self.get_file_prefix(file_path_filename)
        print(f"sanity check: prefix is: {prefix},suffix is: {extension}")
            
        with open(file_path_filename) as object_xys:
        ...
        #dead_observations, alive_observations=self.split_observations_by_displacements(observations, dead_observations, alive_observations,extension)
        #print(f"from observation parser filename is: {file_path_filename}: observations size: {len(observations)}, {len(dead_observations)}, {len(alive_observations)}")
        ...
        for object_id in alive_observations:
            alive_observations[object_id].sort()
        
        for object_id, items in alive_observations.items():
            assert all(items[i][0] <= items[i + 1][0] for i in range(len(items) - 1)), f"Items for {object_id} are not sorted by frame"
        '''
        
        return observations,prefix,extension
    ...
    def get_observations_labels_stats_by_displacements(self, curr_obs, extension):
        ...
        for obj_id, obs in curr_obs.items():
            ...
            if curr_obj_dxdy:
                ...
                #curr_obj_cov = numpy.cov(curr_obj_dxdy_np.T)
                avg_dx,avg_dy=curr_obj_mu[0],curr_obj_mu[1]
                object_avg[obj_id] = (avg_dx, avg_dy)
                '''
                std_dx = numpy.std(curr_obj_dxdy_np[:, 0])
                std_dy = numpy.std(curr_obj_dxdy_np[:, 1])
                object_avg[obj_id] = (std_dx, std_dy)
                '''
        ...
        global_std_dx=numpy.std(all_dx_dy_np[:, 0])
        global_std_dy=numpy.std(all_dx_dy_np[:, 1])
        dxdy_stats={'mu': all_dx_dy_mu, 'cov': all_dx_dy_cov, 'std_dx':global_std_dx, 'std_dy': global_std_dy }
        #print(f"Global avg max dx: {global_avg_dx:.2f}, dy: {global_avg_dy:.2f}")
        
        #if extension.endswith("t"):        
        ...
        print(f"From split function: extension {extension}, total={len(curr_obs)}, dead={len(dead_obs)}, alive={len(alive_obs)}")
        return dead_obs, alive_obs,dxdy_stats
    
    def get_observations_labels_by_displacements_stats(self, curr_obs, curr_obs_stats,extension):
        ...
        for obj_id, obs in curr_obs.items():
            ...
            if curr_obj_dxdy:
                ...
                object_avg[obj_id] = (std_dx, std_dy)
                '''
        global_avg_dx,global_avg_dy = curr_obs_stats['mu']
        global_std_dx=curr_obs_stats['std_dx']
        global_std_dy=curr_obs_stats['std_dy']
        #if extension.endswith("t"):        
            # Second pass: classify based on global averages
        for obj_id, (avg_dx, avg_dy) in object_avg.items():
            obs = curr_obs[obj_id]
        ...
        '''
            ...
            for obj_id, obs in curr_obs.items():
                if len(obs)>5: 
                   dead_obs[obj_id] = obs
        '''           
        print(f"For test set split functions: extension {extension}, total={len(curr_obs)}, dead={len(dead_obs)}, alive={len(alive_obs)}")
