...
def get_displacements(filelists):
    ...
    for filename in filelists:
        observations = collections.defaultdict(list)
        avg_displacement_observations = collections.defaultdict(list)
        ...
        avg_displacement_observations=split_observations_by_displacements(observations)
...
def split_observations_by_displacements(curr_obs):
    ...
    avg_dx_list = [v[0] for v in object_avg.values()]
    avg_dy_list = [v[1] for v in object_avg.values()]

    # Plot overlaid histograms
    plt.hist(avg_dx_list, bins=20, alpha=0.6, label='Average dx', edgecolor='black')
    plt.hist(avg_dy_list, bins=20, alpha=0.6, label='Average dy', edgecolor='black')

    plt.xlabel('Average Displacement per Object')
    plt.ylabel('Number of Objects')
    plt.title('Histogram of Average dx and dy per Object')
    plt.legend()
    plt.grid(True)
    plt.show()
...
def mean_covariance_plot(grid_mu,grid_cov):
    ...
    for i, (mu_row_item,cov_row_item) in enumerate(zip(grid_mu,grid_cov)):
        for j, (mu_col_item, cov_col_item) in enumerate(zip(mu_row_item, cov_row_item)):
            ...
            ax.plot(mu[0], mu[1], 'ro', label="Mean", markersize=10)
            ...
            # Draw the ellipse
            ellipse_1std = Ellipse(xy=mu, width=width, height=height, angle=angle,edgecolor='blue', linestyle='--', linewidth=4, facecolor='none', label="1 Std Dev")
            ...
            buffer = max(width, height) * 0.75  # or just a fixed buffer (e.g., 5)
            ax.set_xlim(mu[0] - buffer, mu[0] + buffer)
            ax.set_ylim(mu[1] - buffer, mu[1] + buffer)
...
def make_collage():
    ...
    output_file = r"outlier_rid_stat_collage.png"
    ...
    collage.save(output_file, "png", resolution=300.0)
...
def mean_covariance_overlay_plot(grid_mu_alive, grid_cov_alive, grid_mu_dead, grid_cov_dead):
    ...
    for i in range(len(grid_mu_alive)):
        for j in range(len(grid_mu_alive[0])):
            ...
            # Plot alive mean
            ax.plot(mu_alive[0], mu_alive[1], 'go', label="Alive Mean", markersize=10)
            ...
            ellipse_alive = Ellipse(xy=mu_alive, width=width, height=height, angle=angle,
                                    edgecolor='green', linestyle='--', linewidth=4, facecolor='none', label="Alive 1 Std Dev")
            ax.add_patch(ellipse_alive)

            # Plot dead mean
            ax.plot(mu_dead[0], mu_dead[1], 'ro', label="Dead Mean",  markersize=10)
            ...
            ellipse_dead = Ellipse(xy=mu_dead, width=width, height=height, angle=angle,
                                   edgecolor='red', linestyle='-', linewidth=4, facecolor='none', label="Dead 1 Std Dev")
...
def plot_accuracy_window():
    ...
    accuracy_values = [0.685, 0.726, 0.763, 0.770, 0.770, 0.770, 0.770, 0.770, 0.770, 0.770] 
    threshold = [-41.815, -11.600, -6.321, -6.767, -6.767, -6.767, -6.767, -6.767, -6.767, -6.767] 
    precision = [0.363, 0.570,0.406, 0.722, 0.722, 0.722, 0.722, 0.722, 0.722, 0.722] 
    recall = [0.114,0.214,0.281, 0.296,0.234,0.203,0.273,0.273,0.273,0.273]
    f1 =[0.173,0.311,0.406, 0.420,0.322,0.295,0.359, 0.359,0.359,0.359,]
    ...
    plt.ylabel('Threshold Selected By Window')
    plt.title('Threshold vs Window')
    #plt.ylim(0.1, 1.0)
    plt.legend()
    plt.tight_layout()
    plt.show()
