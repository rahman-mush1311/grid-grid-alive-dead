...
class OutlierModelEvaluation:
    ...
    def get_thresholds_from_roc(self,curr_obs):
        ...
        for obj_data in curr_obs.values():
            ...
            true_labels.extend([0 if obj_data[TRUE_LABELS] == ALIVE else 1] * len(obj_data[LOG_PDFS]))
        ...
        for i in range(1, len(roc_thresholds)):
            if round(tpr[i],3) > round(tpr[i - 1],3) or round(fpr[i],3)<round(fpr[i-1],3):
    ...
    def plot_confusion_matrix_outlier_model(self,curr_obs, typeofset):
        ...
        precision = precision_score(true_labels, predicted_labels, pos_label='d', average='binary',zero_division=0)
    ...
    def evaluate_thresholds_window_sizes(self,curr_obs, window_sizes):
        ...
        for window in window_sizes:
                ...
                for obj_id, values in curr_obs.items():
                    ...
                    for i in range(len(log_values) - window + 1):
                        ...
                        if all(p <= threshold for p in w):
                            cls = ALIVE
                    predictions.append(0 if cls == ALIVE else 1)
                    true_labels.append(0 if values[TRUE_LABELS] == ALIVE else 1)
            ...
            print(f"for window: {window} window_reset {self.window_size} {accuracy:<10.3f}{f1:<10.3f}{recall:<10.3f}{precision:<10.3f}\n"
                   f"Accuracy Threshold: {self.best_accuracy_threshold}")      
        ...
        for obj_id in extracted_ids:        
            if '1-6-25_274_1' == obj_id:
                ...
                if obj_id in observations:
                    ...
                    y = [a + ((p[2] - y_min) * (b - a)) / (y_max - y_min) for p in points]
                    '''
